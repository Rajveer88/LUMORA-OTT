...
export default function Movies() {
  ...
  const movies = useMemo(() => {
    let base = MOCK_CONTENT.filter(c => c.type === 'movie');
    if (activeGenre) {
      base = base.filter(m => m.genre.includes(activeGenre));
    }
    return base;
  }, [activeGenre]);

  const featuredItems = useMemo(() => {
    return [...movies].sort((a, b) => b.matchPercentage - a.matchPercentage).slice(0, 5);
  }, [movies]);
  ...
  const rows = useMemo(() => {
    ...
    return [
      { id: 'all', title: activeGenre ? `${activeGenre} Movies` : 'All Cinematic Experiences', items: movies },
      { id: 'scifi', title: 'Deep Space & Cybernetics', items: movies.filter(m => m.tags.includes('Sci-Fi')) },
      { id: 'high-rated', title: 'Critically Acclaimed', items: movies.filter(m => parseFloat(m.rating) >= 9.0) },
      ...
      <div className="relative z-30 -mt-16 md:-mt-32 pb-24">
        ...
        {rows.length > 0 ? (
          ...
          <div className="px-6 md:px-12 py-20 text-center">
            <p className="text-white/20 font-black tracking-widest uppercase">No movies found in this frequency</p>
          </div>
        )}
      </div>
    </div>
